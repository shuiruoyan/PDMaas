...
App = function(editor, container, lightbox)
{
	...
	this.editor.addListener('undo', function(editor, evt){
		console.log('===')
	})
	...
	this.editor.graph.container.onscroll = function(e){
		console.log('=====', e)
	}
	this.editor.graph.view.addListener(mxEvent.SCALE, (arg) => {
		console.log('scale', arg)
	});
		...
		window.addEventListener('message', function (e) {
			if(e.data.id === id) {
				if(e.data.status === 'load') {
					drafts.push({
						// created: 1717811832682,
						// key: ".draft_STSG3WDGwUPaetISP4fl",
						// modified: 1717811843543,
						// title: "",
						// type: "draft",
						data: e.data.data
					})
					...
					that.loadDraft(e.data.data)
				}
			}
		})
		window.parent.postMessage({status: 'load', id})
	}
	catch (e)
	{
		result();
		...
		window.setTimeout(mxUtils.bind(this, function()
		{
			...
			this.filterDrafts(null, guid, mxUtils.bind(this, function(drafts)
			{
				if (drafts.length == 1)
				{
					...
					{
						var graph = this.editor.graph
						if (graph.isEnabled() && urlParams['readonly'] == '1')
						{
							var child = graph.model.root
