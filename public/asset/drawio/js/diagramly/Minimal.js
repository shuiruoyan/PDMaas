...
EditorUi.initMinimalTheme = function()
{
	...
	EditorUi.prototype.toggleShapesPanel = function(visible)
	{
		...
		{
			this.sidebarWindow.window.setVisible((visible != null) ?
				visible : !this.sidebarWindow.window.isVisible());
		}
		else
		{
			toggleShapes(this);
	...
	EditorUi.prototype.init = function()
	{
		...
		{
			urlParams['readonly'] != '1' && toggleShapes(this, true);
		...
		{
			urlParams['readonly'] != '1' && toggleFormat(this, true);
			urlParams['readonly'] != '1' && this.formatWindow.window.setVisible(true);
		}
        
		// Needed for creating elements in Format panel
		var ui = this;
		var graph = ui.editor.graph;
		ui.toolbar = this.createToolbar(ui.createDiv('geToolbar geMenuItem'));
        ...
        function createGroup(btns, op, container)
        {
			...
			if (ui.statusContainer != null)
            {
				urlParams['readonly'] != '1' && menubar.insertBefore(btnGroup, ui.statusContainer);
            }
            else
            {
        ...
        function refreshMenu()
        {
			...
			ui.toolbar.container.style.padding = '4px 6px';
			urlParams['readonly'] != '1' &&  menubar.insertBefore(ui.toolbar.container, ui.statusContainer)
        };
