	...
	this.cellRenderer.initializeLabel = function(state, shape)
	{
		...
		mxEvent.addListener(shape.node, 'click', function(evt)
		{
			...
			if(target.tagName === 'A') {
				...
				window.parent.postMessage({status: 'cellLinkClick', id, data: target.getAttribute('href')})
			}
			mxEvent.consume(evt);
		});
	};
...
{
	if (url != null && !url.startsWith('in:') && this.isRelativeUrl(url))
	{
		if (url.charAt(0) == '#')
		{
...
if (typeof mxVertexHandler !== 'undefined')
{
	(function()
	{
		...
		Graph.prototype.createLinkForHint = function(link, label, associatedCell)
		{
			...
			{
				const id = urlParams['id'];
				window.parent.postMessage({status: 'cellLinkClick', id, data: link})
				mxEvent.consume(evt);
			}));
