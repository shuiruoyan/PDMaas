      ...
      to: tempArray[step < 0 ? fieldIndex + step : fieldIndex + step],
        ...
        .forEach((field) => {
          tempArray = moveArrayPositionByFuc(
              ...
              }, step < 0 ? (status === WS.FIELD.MOVE_TOP ? field.fieldIndex - minIndex : field.fieldIndex + step) :
                  (status === WS.FIELD.MOVE_BOTTOM ? field.fieldIndex + (tempArray.length - 1 - maxIndex) : field.fieldIndex + step));
        });
    return tempArray;
  }
  return tempArray;
