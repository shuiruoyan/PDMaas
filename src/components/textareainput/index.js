...
const TextareaInput = React.memo(forwardRef(({cols = 20,
                                          defaultValue, suffix, placeholder, readOnly,
                                       onClick, style, trim, autoFocus,
  ...
  const currentPrefix = getPrefix('components-textarea-input');
  const _onKeyDown = (e) => {
  ...
  return (<span style={style} className={`${currentPrefix} ${suffix ? `${currentPrefix}-suffix-container` : ''}`}>
    ...
    {showDetails && <Tooltip force placement='top' trigger='click' title={renderDetail(tempValue === 0 ? 0 : (tempValue || ''), finalReadOnly)}>
      <span
