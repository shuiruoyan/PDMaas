...
import up from '../../style/up.png';
import down from '../../style/down.png';
...
export default ({ node, graph }) => {
    ...
    const [showFields, hiddenFields] = useMemo(() => {
        const allFields = primaryFields.map((f) => {
            return {
                ...f,
                __type: 'primary',
            };
    ...
    const resetFieldsCount = () => {
        ...
        if(originDataRef.current.fields.length > fieldCount) {
            // 节点大小无法完全显示所有字段时 需要预留21px操作图标空间
    ...
    useEffect(() => {
        ...
        if(autoSize) {
            const {nodeSize} = {...size };
            node.size(nodeSize.width, nodeSize.height, {ignore: isInit.current});
        }
        allFieldSizeRef.current = size.allFieldSize;
        setMaxFieldSizeDefaultState(size.maxFieldSize);
        //setMaxFieldSizeState(size.maxFieldSize);
        // 初始化
        resetFieldsCount();
        resetFieldsAndPorts();
        isInit.current = true;
    ...
    useEffect(() => {
        const handler = {
            ...
            source: ({current, options, edge}) => {
                if(entityRelationRank === 'F'){
                    if(current.cell === node.id && (options.relation ||
                        options.type === 'redo' || options.type === 'undo'
            ...
            target: ({current, previous, options}) => {
                if((current.cell === node.id || previous.cell === node.id) &&
                    (options.type === 'undo' || options.type === 'redo' ||
                        options.isWs || options.relation)) {
                    // 更新字段排序和锚点位置（主要是撤销重做）
                    resetFieldsAndPorts();
    ...
    useEffect(() => {
        ...
        if(entityRelationRank === 'F'){
            ...
            const size = node.size();
            const tempHiddenFields = isExpand ? hiddenFields : normalFields;
            const hindFieldsPort = tempHiddenFields.reduce((p, n) => {
    ...
    }, [hiddenFields, isExpand, normalFields]);
    ...
    const TooltipTitle = () => {
        return <div style={{maxHeight: 400, overflowY: 'auto', width: node.size().width, wordWrap: 'break-word'}}>
          {isExpand ? hiddenFields.map((f, i) => {
                return <div key={f.id}>
                  {f.__type === 'normal'
                        && hiddenFields[i - 1]?.__type !== 'normal'
                        && <div style={{height: 1, background: '#DFE3EB'}}/>}
                  {renderFieldRow(f, f.__type, true)}
                </div>;
            }) : normalFields.map(f => f.defName).join(',')}
        </div>;
    };
    const clampHeight = node.size().height - 26 - 1 - 2 -
        26 * (primaryFields.length + foreignFields.length);
    const tempShowFields = useMemo(() => {
        return isExpand ? showFields : primaryFields.map(f => ({...f, __type: 'primary'}))
            .concat(foreignFields.map(f => ({...f, __type: 'foreign'})));
    }, [isExpand, showFields, primaryFields, foreignFields]);
    useEffect(() => {
    ...
    return (
      <div
        ...
        <div
          ...
          <span
            style={{cursor: 'pointer', position: 'absolute', right: 5, top: 0}}
            onClick={_setExpand}
              >
            <img alt='' src={isExpand ? up : down} style={{width: 20, marginTop: 8}}/>
          </span>
        </div>
        <div>
          {
                  tempShowFields.map((f, i) => {
                      return <div key={f.id}>
                        {f.__type === 'normal'
                              && tempShowFields[i - 1]?.__type !== 'normal'
                              && <div style={{
        ...
        {isExpand ? originDataRef.current.fields.length > showFieldCount && <div style={{
          ...
          >
            <div
              style={{
                        backgroundColor: (fieldHighlight === 'more_in' || fieldHighlight === 'more_out') ? '#DFE3EB' : '#fff',
                        display: '-webkit-box',
                        ...
                        textOverflow: 'ellipsis',
                        wordWrap: 'break-word',
                        color: typeRefStyle.normal.text.fill,
                        fontStyle: typeRefStyle.normal.text['font-style'],

              }}
                >
              {
                entityDisplay.logicModelCompactShow === 'N' ?
                  normalFields.map(f => f.defName).join(entityDisplay.logicModelCompactDelimiter) :
                  normalFields.map(f => f.defKey).join(entityDisplay.logicModelCompactDelimiter)
              }
            </div>
          </Tooltip>
        </div>}
