...
export default ({ node, graph }) => {
    ...
    const { maxFieldSize, showFields, hiddenFields, primaryFields, normalFields, foreignFields } = node.prop('fieldsData');
    ...
    const renderFieldRow = (f, type) => {
        ...
        return <div
          style={{
                background: fieldMark.bgColor || opacity(typeRefStyle[type].body.fill, typeRefStyle[type].body['fill-opacity']),
    ...
    return (
      <div
        ...
        <div>
          {
                  (isExpand ? showFields : primaryFields.map(f => ({...f, __type: 'primary'}))
                      .concat(foreignFields.map(f => ({...f, __type: 'foreign'})))).map((f, i) => {
                      return <div key={f.id}>
                        {f.__type === 'normal'
                                && showFields[i - 1]?.__type !== 'normal'
                                && <div style={{borderBottom: `${divideLineStyle.body['stroke-width']}px 
        ...
        {isExpand ? hiddenFields.length > 0 && <div style={{
            ...
            >
              {
                    entityDisplay.logicModelCompactShow === 'N' ?
                        normalFields.map(f => f.defName)
                            .join(entityDisplay.logicModelCompactDelimiter) :
                        normalFields.map(f => f.defKey)
                            .join(entityDisplay.logicModelCompactDelimiter)
                }
            </div>
          </div>}
