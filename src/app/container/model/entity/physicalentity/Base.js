...
import {physicNsKey} from '../../../../../lib/permission';
...
export default React.memo(forwardRef(({defaultData, onCheck,
    ...
    const [baseData, setBaseData] = useState(_.pick(defaultData, ['defKey', 'defName', 'intro', 'schemaName']));
    const [disable, setDisable] = useState(true);
    const isView = useContext(ViewContent);
    const currentPrefix = getPrefix('container-model-entity-physical-base');
    ...
    return <div className={currentPrefix}>
      {
            baseData.schemaName && <div style={{paddingTop: 5}}>
      ...
      <div>
        ...
        <span className={`${currentPrefix}-item`}>
          ...
          <span>
            <Input
              disable={disable}
              onChange={e => _onValueChange(e, 'defName')}
              nsKey={physicNsKey.U}
              value={baseData.defName}/>
          </span>
        </span>
