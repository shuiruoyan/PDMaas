...
export default React.memo(({getCurrentDataSource, treeData}) => {
    ...
    const exportMarkdown = async (filterData) => {
        ...
        openLoading('正在生成markdown...');
        postWorkerFuc('utils.exportMarkdown', true, [tempDataSource, images, getPermissionList(), getCache('user', true)]).then((data) => {
            downloadBlob(new Blob([data], { type: 'text/markdown;charset=utf-8' }), `${dataSource.project.name}.md`);
        }).catch((e) => {
            console.log(e);
        }).finally(() => {
            closeLoading();
